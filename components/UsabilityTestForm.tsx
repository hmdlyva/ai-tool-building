...
interface UsabilityTestFormProps {
    context: string;
...
export const UsabilityTestForm: React.FC<UsabilityTestFormProps> = ({
    context,
    setContext,
    screenshot,
    setScreenshot,
    ...
    const isGenerateDisabled = isLoading || !context.trim() || !screenshot;
        ...
        <div className="bg-gray-800/50 p-6 rounded-lg border border-gray-700/50 shadow-lg flex flex-col gap-6">
            <div>
                ...
                <textarea
                    ...
                    placeholder="Describe the page's purpose and user goals. E.g., 'This is the homepage for a travel booking site. The goal is for users to find and book a flight.'"
                    value={context}
            ...
            {previewUrl && (
                <div>
                    <p className="block text-sm font-medium text-gray-300 mb-2">Preview:</p>
                    <div className="mt-2 border-2 border-dashed border-gray-600 rounded-lg p-2">
                         <img src={previewUrl} alt="Screenshot preview" className="w-full h-auto rounded-md" />
                    </div>
                </div>
            )}
